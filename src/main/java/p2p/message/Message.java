...
public class Message {
	...
	public enum Type {
		 ...
		 * <li>for {@link RPC.Commands#NEIGHBOR} means check for put (no digest) for tracker and storage</li>
		 * <li>for TASK is submit new task</li>
		 * <li>for {@link RPC.Commands#RELAY} is for setup</li>
		 * <li>for {@link RPC.Commands#RCON} means forward reverse connection to unreachable peer</li>
		 * </ul>
		 */
		REQUEST_1,
		...
		/**
		 * <ul>
		 * <li>for {@link RPC.Commands#GET} returns the extended digest (hashes of all stored data)</li>
		 * <li>for {@link RPC.Commands#PUT}/{@link RPC.Commands#ADD}/COMPARE_PUT means protect domain</li>
		 * <li>for {@link RPC.Commands#REMOVE} means send back results</li>
		 * <li>for RAW_DATA means serialize object</li>
		 * <li>for {@link RPC.Commands#NEIGHBOR} means check for get (with digest) for storage</li>
		 * <li>for TASK is status</li>
		 * <li>for {@link RPC.Commands#RELAY} means send piggybacked message</li>
		 * <li>for {@link RPC.Commands#RCON} open TCP channel and transmit {@link PeerConnection}</li>
		 * </ul>
		 */
		REQUEST_2,
		...
		/**
		 * <ul>
		 * <li>for {@link RPC.Commands#GET} returns a Bloom filter</li>
		 * <li>for {@link RPC.Commands#PUT} means put if absent</li>
		 ...
		 * <li>for {@link RPC.Commands#RELAY} means update the routing table</li>
		 * <li>for {@link RPC.Commands#RCON} use open {@link PeerConnection} to transmit original message</li>
		 * </ul>
		 */
		REQUEST_3,
		...
		/**
		 * <ul>
		 * <li>for {@link RPC.Commands#GET} returns a range (min/max)</li>
		 * <li>for {@link RPC.Commands#PUT} for PUT means protect domain and put if absent</li>
		 * <li>for COMPARE_PUT means partial and protect domain</li>
		 * <li>for {@link RPC.Commands#NEIGHBOR} means check for put (with digest) for task</li>
		 * <li>for {@link RPC.Commands#RELAY} fetch the buffer from the relay peer (Android only)</li>
		 * </ul>
		 */
		REQUEST_4,
		...
		/**
		 * <ul>
		 * <li>for {@link RPC.Commands#RELAY} means that a late response arrived at the relay peer (slow peers
		 * only)</li>
		 * </ul>
		 */
		REQUEST_5,
		...
		/**
		 * <ul>
		 * <li>for {@link RPC.Commands#PEX} means fire and forget, coming from mesh</li>
		 * </ul>
		 */
		REQUEST_FF_1,
		...
		/**
		 * <ul>
		 * <li>for {@link RPC.Commands#PEX} means fire and forget, coming from primary</li>
		 * </ul>
		 */
		REQUEST_FF_2,
    ...
    private List<PeerSocketAddress.PeerSocket4Address> peerSocket4AddressList = null;
    private List<PeerSocketAddress.PeerSocket6Address> peerSocket6AddressList = null;
    ...
    public Message peerSocketAddress(final PeerSocketAddress peerSocketAddress) {
    	if(peerSocketAddress instanceof PeerSocketAddress.PeerSocket4Address) {
    		return peerSocket4Address((PeerSocketAddress.PeerSocket4Address)peerSocketAddress);
    	} else {
    		return peerSocket6Address((PeerSocketAddress.PeerSocket6Address)peerSocketAddress);
    	}
    }
    ...
    public Message peerSocket4Address(final PeerSocketAddress.PeerSocket4Address peerSocket4Address) {
    	if (!presetContentTypes) {
            contentType(Content.PEER_SOCKET4);
        }
    	if(peerSocket4AddressList == null) {
    		peerSocket4AddressList = new ArrayList<PeerSocketAddress.PeerSocket4Address>(1);
    	}
    	peerSocket4AddressList.add(peerSocket4Address);
        return this;
    }

    public List<PeerSocketAddress.PeerSocket4Address> peerSocket4AddressList() {
    	if (peerSocket4AddressList == null) {
            return Collections.emptyList();
        }
    ...
    public PeerSocketAddress.PeerSocket4Address peerSocket4Address(final int index) {
    ...
    public Message peerSocket6Address(final PeerSocketAddress.PeerSocket6Address peerSocket6Address) {
    	if (!presetContentTypes) {
            contentType(Content.PEER_SOCKET6);
        }
    	if(peerSocket6AddressList == null) {
    		peerSocket6AddressList = new ArrayList<PeerSocketAddress.PeerSocket6Address>(1);
    	}
    	peerSocket6AddressList.add(peerSocket6Address);
        return this;
    }

    public List<PeerSocketAddress.PeerSocket6Address> peerSocket6AddressList() {
    	if (peerSocket6AddressList == null) {
            return Collections.emptyList();
        }
    ...
    public PeerSocketAddress.PeerSocket6Address peerSocket6Address(final int index) {
	...
	public int estimateSize() {
		...
		if(peerSocket4AddressList != null) {
			current += (peerSocket4AddressList.size() * PeerSocketAddress.PeerSocket4Address.SIZE);
		}
		
		if(peerSocket6AddressList != null) {
			current += (peerSocket6AddressList.size() * PeerSocketAddress.PeerSocket6Address.SIZE);
		}
