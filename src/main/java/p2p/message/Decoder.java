...
import trunk.social.p2p.connection.TimeoutFactory;
...
public class Decoder {
	...
	public boolean decodeHeader(final ByteBuf buf, InetSocketAddress recipient, final InetSocketAddress sender) {
		...
		for (Message.Content content : message.contentTypes()) {
	...
	public boolean decodePayload(final ByteBuf buf) throws NoSuchAlgorithmException, InvalidKeySpecException,
		...
		while (contentTypes.size() > 0) {
			Message.Content content = contentTypes.peek();
			...
			case SET_NEIGHBORS:
				...
				break;
			case PEER_SOCKET4:
				if (buf.readableBytes() < PeerSocketAddress.PeerSocket4Address.SIZE) {
					return false;
				}
				message.peerSocket4Address(PeerSocketAddress.PeerSocket4Address.decode(buf));
				lastContent = contentTypes.poll();
				break;
			case PEER_SOCKET6:
				if (buf.readableBytes() < PeerSocketAddress.PeerSocket6Address.SIZE) {
					return false;
				}
				message.peerSocket6Address(PeerSocketAddress.PeerSocket6Address.decode(buf));
				lastContent = contentTypes.poll();
				break;
			case SET_KEY640:
