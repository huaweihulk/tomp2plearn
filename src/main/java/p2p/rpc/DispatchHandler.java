...
public abstract class DispatchHandler {
     ...
     */
    public Message createResponseMessage(final Message requestMessage, final Message.Type replyType) {
        return createResponseMessage(requestMessage, replyType, peerBean().serverPeerAddress());
    }
    
    public static Message createResponseMessage(final Message requestMessage, final Message.Type replyType, final PeerAddress peerAddress) {
    ...
    public void forwardMessage(final Message requestMessage, PeerConnection peerConnection, Responder responder) {
    	...
    	if(requestMessage.sender().net4Internal() || 
    			(requestMessage.type() == Message.Type.REQUEST_1 && requestMessage.command() == RPC.Commands.RELAY.getNr()) ||
    			(requestMessage.type() == Message.Type.REQUEST_2 && requestMessage.command() == RPC.Commands.PING.getNr()) ) {
