...
import trunk.social.p2p.p2p.PeerReceivedBroadcastPing;
...
public class PingRPC extends DispatchHandler {
	...
	public RequestHandler<FutureResponse> ping(final PeerAddress remotePeer, final ConnectionConfiguration configuration) {
		return createHandler(remotePeer, Message.Type.REQUEST_1, configuration);
	}
	...
	public FutureResponse pingBroadcastUDP(final PeerAddress remotePeer, final ChannelCreator channelCreator,
			final ConnectionConfiguration configuration) {
		return createHandler(remotePeer, Message.Type.REQUEST_4, configuration).sendBroadcastUDP(channelCreator);
	}
	...
	public FutureResponse fireUDP(final PeerAddress remotePeer, final ChannelCreator channelCreator,
			final ConnectionConfiguration configuration) {
		return createHandler(remotePeer, Message.Type.REQUEST_FF_1, configuration).fireAndForgetUDP(channelCreator);
	}
	...
	public FutureResponse fireTCP(final PeerAddress remotePeer, final ChannelCreator channelCreator,
			final ConnectionConfiguration configuration) {
		return createHandler(remotePeer, Message.Type.REQUEST_FF_1, configuration).sendTCP(channelCreator);
	}
	...
	public FutureResponse pingUDPProbe(final PeerAddress remotePeer, final ChannelCreator channelCreator,
			final ConnectionConfiguration configuration) {
		final Message message = createMessage(remotePeer, RPC.Commands.PING.getNr(), Message.Type.REQUEST_3);
	 ...
	 */
	private RequestHandler<FutureResponse> createHandler(final PeerAddress remotePeer, final Message.Type type,
			final ConnectionConfiguration configuration) {
	...
	private FutureResponse createDiscoverHandler(final PeerAddress remotePeer) {
		final Message message = createMessage(remotePeer, RPC.Commands.PING.getNr(), Message.Type.REQUEST_2);
	...
	public void handleResponse(final Message message, PeerConnection peerConnection, final boolean sign,
		...
		if (!((message.type() == Message.Type.REQUEST_FF_1 || message.type() == Message.Type.REQUEST_1
				|| message.type() == Message.Type.REQUEST_2 || message.type() == Message.Type.REQUEST_3
				|| message.type() == Message.Type.REQUEST_4) && message.command() == RPC.Commands.PING
				.getNr())) {
		...
		if (message.type() == Message.Type.REQUEST_3) {
			...
			if(message.isSendSelf()) {
				responseMessage = createResponseMessage(message, Message.Type.NOT_FOUND);
				LOG.warn("Sending probe ping request to yourself? If those are two different peers, messages may be dropped");
			} else {
				responseMessage = createResponseMessage(message, Message.Type.OK);
		...
		} else if (message.type() == Message.Type.REQUEST_2) { // discover
		...
		} else if (message.type() == Message.Type.REQUEST_1 || message.type() == Message.Type.REQUEST_4) { // regular
																							// ping
