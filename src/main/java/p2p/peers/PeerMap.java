...
import trunk.social.p2p.p2p.PeerStatisticComparator;
import trunk.social.p2p.utils.CacheMap;
...
public class PeerMap implements PeerStatusListener, Maintainable {
    ...
    public boolean peerFailed(final PeerAddress remotePeer, final PeerException peerException) {
        ...
        if (updatePeerStatistic(remotePeer, peerMapVerified.get(classMember), offlineCount)) {
            return peerFailed(remotePeer, new PeerException(PeerException.AbortCause.PROBABLY_OFFLINE, "Peer failed in verified map."));
        }
        if (updatePeerStatistic(remotePeer, peerMapOverflow.get(classMember), offlineCount)) {
            return peerFailed(remotePeer, new PeerException(PeerException.AbortCause.PROBABLY_OFFLINE, "Peer failed in overflow map."));
        }
        return false;
    }
