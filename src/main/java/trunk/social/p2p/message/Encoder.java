...
public class Encoder {
    ...
    private boolean loop(AlternativeCompositeByteBuf buf) throws InvalidKeyException, SignatureException, IOException {
        ...
        while ((next = message.contentReferences().peek()) != null) {
        	...
        	final Message.Content content = next.content();
            switch (content) {
            case KEY:
            ...
            case SET_NEIGHBORS:
                ...
                break;
            case PEER_SOCKET4:
            	final PeerSocketAddress.PeerSocket4Address psa4 = message.peerSocket4Address(next.index());
            	psa4.encode(buf);
                message.contentReferences().poll();
                break;
            case PEER_SOCKET6:
            	final PeerSocketAddress.PeerSocket6Address psa6 = message.peerSocket6Address(next.index());
            	psa6.encode(buf);
                message.contentReferences().poll();
                break;
            case BLOOM_FILTER:
            ...
            case MAP_KEY640_DATA:
                ...
                if (dataMap.isConvert()) {
                    for (final Entry<Number160, Data> entry : dataMap.dataMapConvert().entrySet()) {
                    	...
                    	encodeData(buf, entry.getValue(), dataMap.isConvertMeta(), !message.isRequest(), message.command() == RPC.Commands.REPLICA_PUT.getNr());
                    }
                } else {
            ...
            case SET_TRACKER_DATA:
                ...
                for (final Entry<PeerAddress, Data> entry : trackerData.peerAddresses().entrySet()) {
                	entry.getKey().encode(buf);
                	final Data data = entry.getValue().duplicate();
                    encodeData(buf, data, false, !message.isRequest(), message.command() == RPC.Commands.REPLICA_PUT.getNr());
                }
                message.contentReferences().poll();
                break;
