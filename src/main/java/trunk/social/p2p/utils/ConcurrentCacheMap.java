 ...
 * the License.
 */
package trunk.social.p2p.utils;
...
public class ConcurrentCacheMap<K, V> implements ConcurrentMap<K, V> {
    ...
    public Set<Entry<K, V>> entrySet() {
        final Set<Entry<K, V>> retVal = new HashSet<Entry<K, V>>() {
            private static final long serialVersionUID = 3769009451779243542L;

			@Override
        	public Iterator<Entry<K, V>> iterator() {
				final Iterator<Entry<K,V>> orig = super.iterator();
        ...
        for (final CacheMap<K, ExpiringObject> segment : segments) {
            synchronized (segment) {
                final Iterator<Entry<K, ExpiringObject>> iterator = segment.entrySet().iterator();
                while (iterator.hasNext()) {
                    final Entry<K, ExpiringObject> entry = iterator.next();
                    if (entry.getValue().isExpired()) {
                        iterator.remove();
                        LOGGER.debug("Removed in entry set: {}.", entry.getValue().getValue());
                        removedCounter.incrementAndGet();
                    } else {
                        retVal.add(new Entry<K, V>() {
                            @Override
                            public K getKey() {
                                return entry.getKey();
                            }
