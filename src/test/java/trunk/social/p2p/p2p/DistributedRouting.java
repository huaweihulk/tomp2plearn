...
import trunk.social.p2p.futures.FutureForkJoin;
...
public class DistributedRouting {
    ...
    public FutureDone<Pair<FutureRouting,FutureRouting>> bootstrap(final Collection<PeerAddress> peerAddresses,
        ...
        final FutureRouting futureRouting0 = routing(peerMap().getPeerStatistics(peerAddresses), routingBuilder, Message.Type.REQUEST_1, cc);
        ...
        futureRouting0.addListener(new BaseFutureAdapter<FutureRouting>() {
            ...
            public void operationComplete(final FutureRouting future) throws Exception {
            	...
            	if(future.isSuccess()) {
            		...
            		final FutureRouting futureRouting1 = routing(peerMap().getPeerStatistics(peerAddresses), routingBuilder, Message.Type.REQUEST_1, cc);
    ...
    public FutureRouting quit(final RoutingBuilder routingBuilder, final ChannelCreator cc) {
        ...
        return routing(startPeers, routingBuilder, Message.Type.REQUEST_4, cc);
    }
     ...
     * @return a FutureRouting object, is set to complete if the route has been found
     */
    public FutureRouting route(final RoutingBuilder routingBuilder, final Message.Type type, final ChannelCreator cc) {
    ...
    private FutureRouting routing(final Collection<PeerStatistic> peerAddresses,
                                  final RoutingBuilder routingBuilder, final Message.Type type, final ChannelCreator cc) {
        ...
        if (type == Message.Type.REQUEST_2 && routingBuilder.domainKey() != null && !randomSearch && peerBean.digestStorage() !=null) {
        ...
        } else if (type == Message.Type.REQUEST_3 && !randomSearch && peerBean.digestTracker() != null) {
    ...
    private void routingRec(final RoutingBuilder routingBuilder, final RoutingMechanism routingMechanism,
                            final Message.Type type, final ChannelCreator channelCreator) {
