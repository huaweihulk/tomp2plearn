...
public class NeighborRPC extends DispatchHandler {
    ...
    public FutureResponse closeNeighbors(final PeerAddress remotePeer, final SearchValues searchValues,
                                         final Message.Type type, final ChannelCreator channelCreator, final ConnectionConfiguration configuration) {
    ...
    public void handleResponse(final Message message, PeerConnection peerConnection, final boolean sign, Responder responder) throws IOException {
        ...
        if (!(message.type() == Message.Type.REQUEST_1 || message.type() == Message.Type.REQUEST_2
                || message.type() == Message.Type.REQUEST_3 || message.type() == Message.Type.REQUEST_4)
        ...
        if(neighbors == null) {
            ...
            Message response = createResponseMessage(message, Message.Type.NOT_FOUND);
        ...
        boolean isDigest = message.type() != Message.Type.REQUEST_1;
        if (isDigest) {
